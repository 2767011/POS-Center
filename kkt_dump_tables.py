...
def create_driver():
    """Создание COM-объекта драйвера ККТ."""
    errors = []
    for prog_id in COM_PROG_IDS:
        try:
            ...
            errors.append(f"  {prog_id}: нет свойства Password")
        except Exception as e:
            errors.append(f"  {prog_id}: {e}")
    bits = struct.calcsize("P") * 8
    print(f"Ошибка: не удалось создать COM-объект AddIn.DrvFR ({bits}-bit Python)")
    for err in errors:
        print(err)
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip", default="192.168.137.111",
                        help="IP-адрес ККТ (default: 192.168.137.111)")
    parser.add_argument("--port", type=int, default=7778,
                        help="TCP порт ККТ (default: 7778)")
    ...
    parser.add_argument("--timeout", type=int, default=5000,
                        help="Таймаут подключения мс (default: 5000)")
